...
namespace MongoIdentitySample.Mvc.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (rolesuser.Contains("Trader"))
                    {
                        return RedirectToAction("Create", "TaxInvoice");
                    }
                    ////InviteUser/InviteUsers
                    if (rolesuser.Contains("TaxProfessionals"))
                    {
                        return RedirectToAction("InviteUsers", "InviteUser");
                    }
                    if (rolesuser.Contains("TaxRegulator"))
                    {
                        return RedirectToAction("Regulator", "ViewInvoice");
                    }
        ...
        public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (ut.SearchFor(x => x.Token == model.Token && x.Created == false).Any())
                {
                    ...
                    var user = new ApplicationUser { UserName = model.Email, Email = model.Email, EntityName = model.EntityName, Address1 = model.Address1, Address2 = model.Address2, PINCode = model.PINCode, GstId = model.GstId };
        ...
        {
            //MongoDbRepository<> restaurantRepo = new MongoDbRepository<>();
